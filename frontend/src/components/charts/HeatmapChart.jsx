...
function HeatmapChart({ data }) {
  ...
  useEffect(() => {
    ...
    const groupedData = d3.group(topStocks, d => d.industry || '熱門股票');
    ...
    industries.forEach(industry => {
      ...
      group.selectAll('rect')
        ...
        .attr('fill', d => {
          const change = d.change || (d.change_percent || 0) / 100;
        ...
        .on('mouseover', function(event, d) {
          ...
          const tooltip = d3.select('body').append('div')
            ...
            .html(`股票: ${d.stock_id}<br>名稱: ${d.stock_name || '未知'}<br>漲跌幅: ${((d.change || (d.change_percent || 0) / 100) * 100).toFixed(2)}%<br>關注度: ${d.popularity_score?.toFixed(2) || 'N/A'}`);
