...
function MarketOverview() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        console.log('開始獲取市場概覽數據');
        const data = await getMarketOverview();
        console.log('市場概覽 API 回應:', data);
        if (data.length === 0) {
          setError('無市場概覽數據，請確認後端 API 或 stock_features 表是否有最新數據。');
        }
        setStocks(data);
        setLoading(false);
        console.log('市場概覽數據獲取成功');
      } catch (error) {
        console.error(`獲取市場概覽數據失敗: ${error.message}`);
        setError('無法連接到後端 API，請檢查 http://localhost:8000/market-overview 或後端服務是否正常運行。');
        setStocks([]);
        setLoading(false);
      }
    }
  ...
  return (
    <div className="card">
      <h1 className="text-3xl font-bold mb-6 text-text">市場概覽</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {topStocks.map(stock => (
              <div key={stock.stock_id} className="bg-gray-700 p-4 rounded-lg shadow">
                ...
                <p className={stock.change_percent >= 0 ? 'text-up' : 'text-down'}>
